...
###############################################################################
sub main() {
    my $cycles_to_run = 320000000;
    my $max_threads = 16;
    my $output_filename = "output2.txt";
    my $single_thread_time;

    open(my $HW2_output_file, '>', $output_filename) or die "ERROR: Could not open $output_filename for write.";
    
    for(my $num_threads = 1; $num_threads <= $max_threads; $num_threads++) {
        my $cmd = "./HW2 $cycles_to_run $num_threads";
        ...
        print $HW2_output_file "$num_threads\t$runtime_secs\t$speedup\n";
    }
    close($HW2_output_file);
}
