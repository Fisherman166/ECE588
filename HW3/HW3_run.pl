...
sub main() {
    my $max_threads = 31;
    my $output_filename = "output3.txt";
    my $single_thread_time;

    open(my $HW3_output_file, '>', $output_filename) or die "ERROR: Could not open $output_filename for write.";
    
    for(my $num_threads = 1; $num_threads <= $max_threads; $num_threads++) {
        my $cmd = "./HW3 $num_threads";
        ...
        print $HW3_output_file "$num_threads\t$runtime_secs\t$speedup\n";
    }
    close($HW3_output_file);
}
